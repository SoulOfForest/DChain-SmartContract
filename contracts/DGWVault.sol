...
contract DGWVault is DChainBase {
    ...
    function setAllowToStakeUsingLockedBalance(bool _allowToStakeUsingLockedBalance) external onlyRole(SUB_ADMIN_ROLE) {
    ...
    function stakeWithVault(
        ...
        if (_lockedAmount > 0 && availableToStakeThroughVault(sender)) {
            require(allowToStakeUsingLockedBalance, "pool: not allow to stake using locked balance");
    ...
    function setOfferedCurrency(address _currency, uint _rate, uint _decimals) external onlyRole(SUB_ADMIN_ROLE) {
    ...
    function availableToStakeThroughVault(address _user) public view returns(bool) {
    ...
    function getSoldTokenFromOfferedCurrency(address _token, uint _amount) public view returns(uint) {
